...
// Create does not seize a credential this configuration never made: without
// adopt_existing it fails with the import command, with it the PATCH takes
// the credential over. See https://github.com/BerriAI/terraform-provider-litellm/issues/8.
	...
	if !d.Get("adopt_existing").(bool) {
		return fmt.Errorf(
			"credential %q already exists on the proxy but is not in Terraform state. "+
				...
				"  terraform import litellm_credential.<this resource's name in your config> %q\n\n"+
